...
RealType Scenarios::BathymetryDamBreakScenario::getWaterHeight(RealType x, RealType y) const {
    return RealType(260.0);
}

RealType Scenarios::BathymetryDamBreakScenario::getBathymetry([[maybe_unused]] RealType x,
                                                              [[maybe_unused]] RealType y)
const {
    return RealType(
            (std::sqrt((x - 500.0) * (x - 500.0) + (y - 500.0) * (y - 500.0)) < 50.0) ? -255.0
                                                                                      : -260.0);
}
...
BoundaryType
Scenarios::BathymetryDamBreakScenario::getBoundaryType([[maybe_unused]] BoundaryEdge edge) const {
